...
from src.database_models import Base, User, Category, Subcategory, Item, ItemData, Sale, SoldItem, Payment, Topup, Order, ReferralHistory, UserPricing
...
# # # Кастомные цены (UserPricing) # # #

async def get_custom_price(user_id: int, service: str, country: str) -> Optional[float]:
    """Возвращает кастомную цену для юзера по сервису+стране, или None если нет."""
    async with async_session() as session:
        # Ищем точное совпадение service+country
        result = await session.execute(
            select(UserPricing).where(
                UserPricing.user_id == user_id,
                UserPricing.service == service,
                UserPricing.country == country
            )
        )
        row = result.scalar_one_or_none()
        if row:
            return row.price_usd
        # Ищем wildcard country='*'
        result = await session.execute(
            select(UserPricing).where(
                UserPricing.user_id == user_id,
                UserPricing.service == service,
...
async def set_custom_price(user_id: int, service: str, country: str, price_usd: float):
    """Устанавливает или обновляет кастомную цену."""
    async with async_session() as session:
        result = await session.execute(
            select(UserPricing).where(
                UserPricing.user_id == user_id,
        ...
        if row:
            ...
            session.add(UserPricing(user_id=user_id, service=service, country=country, price_usd=price_usd))
        await session.commit()

async def delete_custom_price(user_id: int, service: str, country: str):
    """Удаляет кастомную цену (вернуть к стандартной)."""
    async with async_session() as session:
        await session.execute(
            delete(UserPricing).where(
                UserPricing.user_id == user_id,
                UserPricing.service == service,
                UserPricing.country == country
            )
...
async def get_all_custom_prices():
    """Все кастомные цены для админки."""
    async with async_session() as session:
        result = await session.execute(select(UserPricing))
        return result.scalars().all()

async def get_user_custom_prices(user_id: int):
    """Кастомные цены конкретного юзера."""
    async with async_session() as session:
        result = await session.execute(
